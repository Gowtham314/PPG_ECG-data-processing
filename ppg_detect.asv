...
tHR_ppgeshold1=2*min_high;
noise_peak=high(slp(high)>tHR_ppgeshold1);
high(slp(high)>tHR_ppgeshold1)=[];

tHR_ppgeshold2=0.76*(max(slp(high))+min(slp(high)))/2;
high(slp(high)<tHR_ppgeshold2)=[];
...
tHR_ppgeshold3=0.4*(min(slp1(peak))+ max(slp1(peak)))/2;
peak(slp1(peak)<tHR_ppgeshold3)=[];
slp1(slp1<tHR_ppgeshold3)=0;
...
HR_ppg=zeros(length(systole_peak),1); % tao mot mang chua cac gia tri hearate
for i=2:length(systole_peak)              % HR_ppg(1)=0
    j=find(noise_peak < systole_peak(i),1,'last');
        if noise_peak(j)>systole_peak(i-1)
            HR_ppg(i)=0;
        else
            HR_ppg(i)=60*fs/(systole_peak(i)-systole_peak(i-1));
        end
end

figure;
stem(HR_ppg,'ro','MarkerEdgeColor','b','MarkerFaceColor','b','MarkerSize',6);
title('Heart Rate');
